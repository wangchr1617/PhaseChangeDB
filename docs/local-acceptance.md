...
uv run pytest
```
- **输出结果**：`27 passed, 3 skipped in 0.22s`（退出码 0）。
- **覆盖内容**：
  - AI 暂存隔离与晋升契约校验；
  - 严格单位归一化校验（同单位解析、非同单位无回退保持 NULL、成对约束与 Decimal 科学精度保存）；
  ...
  - OpenAPI 契约端点完整性与方法覆盖检查。

### 2.4 真实 MySQL 8.4 集成测试
```bash
PCM_INTEGRATION=1 uv run pytest
```
- **输出结果**：`30 passed in 0.34s`（退出码 0，包含全部 27 项单元/契约测试与 3 项真实 MySQL 集成闭环）。
- **验证内容**：
  - MySQL 8.4 就绪与核心种子数据存在；
  - **AI 提取候选暂存、拒绝分支与晋升闭环**：
    1. AI 提交候选至 `/v1/extractions/candidates`，验证仅落入 `ext_*` 暂存表，`obs_observation` 记录数为 0；
    2. 候选拒绝分支测试：验证更新状态为 `rejected`、记录不可变 `ext_review` 且绝不产生 Observation；
    3. `If-Match` 乐观锁测试（格式错误返回 400，版本不匹配返回 412）；
    4. 候选审核晋升（`accept`）：单个事务内原子生成 `HUMAN_REVIEWED` 状态的正式 Observation，`normalized_value` 严格保持为 `NULL`；
    5. 不可变审核记录 `ext_review` 校验与 Reviewer 身份追溯；
    6. 关联证据片段 `evd_fragment` 与 `evd_observation_link` 事务一致性；
    7. 事务性 `sys_audit_log` 与 `sys_outbox_event` 生成；
  - **人工录入工作流闭环**：
...
2. **远程文档上传**：当前接口负责登记和校验 S3/HTTPS 文档的存储 URI 与 SHA-256，尚未接入实际的大文件直传或分片上传协议。
